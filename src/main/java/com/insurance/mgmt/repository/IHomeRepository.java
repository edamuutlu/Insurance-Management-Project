...
public interface IHomeRepository extends JpaRepository<Home,Integer>{ 
    ...
    @Query("SELECT h FROM Home h WHERE h.province = :province AND h.district = :district AND h.neighbourhood = :neighbourhood AND "
    		+ "h.buildingNumber = :buildingNumber AND h.apartment = :apartment AND h.floor = :floor AND h.status = :status")
